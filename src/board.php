...
class Game {
    ...
    function game_play($link) {
        ...
        for ($pos = 0; $pos < pow($this->grid_size, 2); $pos++) {
            ...
            if ($link) {
                ...
                echo '<td style="text-align:center;', ' background-color: #90EE90;' , ' opacity: 0.5;'  . '"><div style="padding: 1em;">' . $this->position[$pos] . ($this->debug ? ('<br /><span style="font-size:60%;">' . $pos . ':(' . $row . ',' . (($pos % $this->grid_size) + 1) . ')</span>') : '') . '</div></td>';
    ...
    function game_message($message) {
        ...
        switch ($message) {
            ...
            case 'new-game':
                ...
                echo 'A new game has started. Since I am the game host, I\'ll let you start first.<br />';
            ...
            case 'x-win':
                ...
                echo '<strong>X (YOU) are the winner of this game.  Congratulations.</strong>';

                break;

            case 'o-win':

                echo '<strong>(O)The computer is the winner of this game.  Care for a rematch?  You\'ll probably lose again haha...</strong>';
    ...
    function game_stats() {
        ...
        $wins  = count(glob("Results/".$_COOKIE['name']. "/*.win"));

        $loses = count(glob("Results/".$_COOKIE['name'].  "/*.lose"));

        $ties  = count(glob("Results/".$_COOKIE['name'].  "/*.tie"));

        $games = count(glob("Results/".$_COOKIE['name'].   "/*.*"));
